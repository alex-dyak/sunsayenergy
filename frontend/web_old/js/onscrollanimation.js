...
if (window.innerWidth > 1024) {
  ...
  setScrollMagicWithHook('.connect', 0.7, TweenMax.to('.connect__content-title', .5, { opacity: 1 }))
  setScrollMagicWithHook('.connect', 0.7, TweenMax.to('.connect__content-question', .5, { opacity: 1 }).delay(.4))
  setScrollMagicWithHook('.connect', 0.7, TweenMax.to('.connect__content-image', .5, { opacity: 1 }).delay(.8))
  setScrollMagicWithHook('.example', 0.7, TweenMax.to('.example__content-title', .5, { opacity: 1 }))
  setScrollMagicWithHook('.example', 0.7, TweenMax.to('.example__content-list', .5, { opacity: 1 }).delay(.4))
  setScrollMagicWithHook('.legislation', 0.7, TweenMax.to('.legislation__content-info', .5, { opacity: 1 }))
  setScrollMagicWithHook('.legislation', 0.7, TweenMax.to('.legislation__content-image', .5, { opacity: 1 }))
  setScrollMagicWithHook('.complex', 0.65, TweenMax.to('.complex__content-title', .5, { opacity: 1 }))
  setScrollMagicWithHook('.info', 0.35, TweenMax.staggerTo('.electrostation__info, .electrostation__contact, .electrostation__switch', 1, { opacity: 0, marginTop: -150 }))
  setScrollMagicWithHook('.type', 0.7, TweenMax.to('.type__content-title', .5, { opacity: 1 }))
  setScrollMagicWithHook('.type__content-list__item:nth-child(1)', 0.65, TweenMax.to('.type__content-list__item:nth-child(1)', .5, { opacity: 1 }))
  setScrollMagicWithHook('.type__content-list__item:nth-child(2)', 0.65, TweenMax.to('.type__content-list__item:nth-child(2)', .5, { opacity: 1 }))
  setScrollMagicWithHook('.type__content-list__item:nth-child(3)', 0.65, TweenMax.to('.type__content-list__item:nth-child(3)', .5, { opacity: 1 }))
  setScrollMagicWithHook('.rate__content-image', 0.7, TweenMax.to('.rate__content-image', .5, { opacity: 1 }))
  setScrollMagicWithHook('.rate', 0.7, TweenMax.to('.rate__content-info > .title', .5, { opacity: 1 }))
  setScrollMagicWithHook('.rate__content-info > .description', 0.7, TweenMax.to('.rate__content-info > .description', .5, { opacity: 1 }))
  setScrollMagicWithHook('.rate__content-info > .button', 0.7, TweenMax.to('.rate__content-info > .button', .5, { opacity: 1 }))
}
...
setScrollMagic('.greenwork',0,TweenMax.to('.work__content-list__item:nth-child(7)',.5,{opacity:1}).delay(.9))})();function setScrollMagic(element,offset,tween){new ScrollMagic.Scene({triggerElement:element,offset:offset,reverse:!0}).setTween(tween).addTo(new ScrollMagic.Controller())}
function animateSVG(element,path,points,delay){var s=Snap(element);var path=s.select(path);var pathPoints=points;path.animate({d:pathPoints},delay,mina.linear)}
function setScrollMagicWithHook(element, hook, tween) { new ScrollMagic.Scene({ triggerElement: element, triggerHook: hook, reverse: !0 }).setTween(tween).addTo(new ScrollMagic.Controller()) }